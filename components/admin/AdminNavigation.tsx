...
export const AdminNavigation: React.FC<AdminNavigationProps> = ({ 
      ...
      >
        <Link href="/admin/posts">
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            className="bg-accent-blend text-primary-foreground p-4 rounded-2xl shadow-lg hover:opacity-90 transition-all flex items-center gap-2 group glass-effect backdrop-blur-sm"
            title="관리자 패널"
          >
            <Settings size={24} />
      ...
      >
        <div className="container mx-auto px-6 py-4">
          <div className="flex items-center gap-2 text-sm">
            <Link
              href="/"
              className="text-foreground-secondary hover:text-foreground transition-all flex items-center gap-1 font-medium hover:scale-105"
            >
              <Home size={16} />
              홈
            </Link>
  ...
  return (
    <motion.aside
      ...
      transition={{ duration: 0.6, ease: [0.25, 0.46, 0.45, 0.94] }}
      className={`bg-background-card border-r border-border/30 min-h-screen shadow-sm ${className}`}
    >
      <div className="p-6">
        {/* Admin Header */}
        <motion.div
              ...
              >
                <Link href={item.href}>
                  <motion.div
                    ...
                    whileTap={{ scale: 0.98 }}
                    className={`flex items-center gap-3 p-4 rounded-2xl transition-all group cursor-pointer glass-effect backdrop-blur-sm ${
                      isActive
