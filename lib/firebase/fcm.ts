...
export async function getCachedAdminFCMToken(): Promise<string | null> {
  console.warn(
    "⚠️ DEPRECATED: getCachedAdminFCMToken() is deprecated. Use getAdminFCMTokenDirectly()"
  );
...
export async function getAdminFCMTokenDirectly(): Promise<string | null> {
  ...
  try {
    ...
    if (!docSnap.exists()) {
      console.error(
        "❌ FCM master document not found in Firestore at fcm/master"
      );
      console.log(
        "💡 Please ensure FCM token is saved via admin panel: /admin/messages"
      );
      return null;
    }
    ...
    if (!token || typeof token !== "string") {
      console.error(
        "❌ Invalid or missing token field in fcm/master document:",
        { hasToken: !!token, tokenType: typeof token }
      );
      return null;
    }
    ...
    console.error(
      "❌ Exception while fetching FCM token from fcm/master document:",
      error
    );
...
export function clearFCMTokenCache(): void {
  ...
  console.log(
    "🧹 FCM token cache cleared - next request will fetch from fcm/master document"
  );
}
...
export async function sendFCMNotification(
  ...
  if (!adminToken) {
    ...
    throw new Error(
      "FCM token not available - check Firestore fcm/master document"
    );
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Custom FCM endpoint error: ${response.status} - ${errorText}`
      );
    }

    const result = await response.text(); // Custom endpoint might return text instead of JSON
    console.log(
      "✅ FCM notification sent successfully via custom endpoint:",
      result
    );
  } catch (error) {
    console.error(
      "❌ Error sending FCM notification via custom endpoint:",
      error
    );
...
export async function saveAdminFCMTokenToFirestore(
  token: string
...
export async function requestNotificationPermissionForAdmin(
  saveAsAdmin: boolean = false
): Promise<string | null> {
  ...
  if (token && saveAsAdmin) {
    ...
    if (currentUser && isMasterUser(currentUser)) {
      ...
      console.warn(
        "⚠️ Cannot save as admin token: user is not authenticated master user"
      );
    }
  }
